...
def load_loss(loss_name, class_weights=None, num_parts=None, printscreen=True, device=None):
    ...
    if loss_name == 'weighted_categorical_crossentropy':
        ...
        loss = DrLIM()
    elif loss_name == 'IoU':  # DRLIM http://yann.lecun.com/exdb/publis/pdf/hadsell-chopra-lecun-06.pdf
        loss = MeanAveragePrecision()
    else:
...
def giou_loss(input_boxes, target_boxes, eps=1e-7):
    ...
    return loss.sum()
def getIoU(boxes_pred, boxes):
    loss = []
    for i in range(len(boxes)):
        size_diff = boxes[i].size(dim=0) - boxes_pred[i].size(dim=0) 
        if size_diff != 0:
            loss.append(1 * abs(size_diff)) #maximum iou difference
        iou = ops.box_iou(boxes_pred[i], boxes[i])
        values, indices = torch.max(iou, dim=0)
        loss.append(i)
    return np.mean(loss)
