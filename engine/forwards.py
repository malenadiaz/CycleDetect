...
def objectDetect(model: torch.nn, data: List, criterion: torch.nn, device: torch.device) -> Tuple[Dict, Dict]:
   
    imgs = list(image.to(device) for image in data[0])
    
    data_good = [data[1]] if type(data[1]) is dict else data[1] 
    boxes = [{k: v.to(device) for k, v in t.items()} for t in data[1]]
...
def run_forward(model: torch.nn, data: List, criterion: torch.nn, device: torch.device) -> Dict:
    ...
    elif model.output_type == 'objectDetect':
        losses, outputs = objectDetect(model, data, criterion, device)
    else:
